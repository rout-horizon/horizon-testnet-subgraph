...
import { Vested as VestedEvent, RewardEscrow } from '../generated/subgraphs/issuance/issuance_RewardEscrow_0/RewardEscrow'
...
export function handleTransferSNX(event: SNXTransferEvent): void {
    ...
    if (event.params.from.toHex() != ZERO_ADDRESS.toHex()) {
      ...
      updateActiveStaker(event.address, event.params.from);
    } else if (synth != null) {
      // snx is minted
...
//   }
  ...
  updateActiveStaker(synthetixAddress, event.params.beneficiary);
}
...
function trackActiveStakers(event: ethereum.Event, isBurn: boolean): void {
  ...
  if (isBurn && activeStaker != null && accountDebtBalance == BigInt.fromI32(0)) {
    ...
    activeStaker.collateral = toDecimal(synthetix.collateral(account));
    activeStaker.debtBalanceOf = toDecimal(accountDebtBalance);
    activeStaker.save();
    totalActiveStaker.count = totalActiveStaker.count.plus(BigInt.fromI32(1));
    totalActiveStaker.save();
  }
  else if (activeStaker != null) {
    updateActiveStaker(snxContract, account);
  }
...
function updateActiveStaker(snxContract: Address, account: Address): void {
  let activeStaker = ActiveStaker.load(account.toHex());
  if (activeStaker != null) {
    let synthetix = SNX.bind(snxContract);
    let collateralTry = synthetix.try_collateral(account);
    if (!collateralTry.reverted) {
      activeStaker.collateral = toDecimal(collateralTry.value);
    }
  
    let debtBalanceOfTry = synthetix.try_debtBalanceOf(account, zUSD32);
    if (!debtBalanceOfTry.reverted) {
      activeStaker.debtBalanceOf = toDecimal(debtBalanceOfTry.value);
    }
