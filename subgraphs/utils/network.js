...
function getCurrentNetwork() {
  return process.env['SNX_NETWORK'] || 'bsc';
}
...
function getReleaseInfo(file, network = undefined) {
  ...
  let info = null;
  if (net === 'bsc') {
    return require('@rout-horizon/horizon-testnet/publish/deployed/mainnet/' + file);
    // return require('synthetix/publish/deployed/' + net + '/' + file);    
  }
  else if (net === 'chapel') {
    return require('@rout-horizon/horizon-testnet/publish/deployed/testnet/' + file);
...
function createSubgraphManifest(name, dataSources, templates) {
  ...
  const manifest = {
    specVersion: '0.0.4',
    features: ['grafting'],
    description: name ? 'Horizon Subgraph' : 'Horizon Subgraph ' + name,
    repository: 'https://github.com/Horizon-Protocol/horizon-subgraph',
    schema: {
      file: `./${name}.graphql`,
    },
