...
program.action(async () => {
  const NETWORK_CHOICES = ['bsc', 'chapel'];
  ...
  for (let i = 0; i < SUBGRAPH_CHOICES.length; i++) {
    ...
    await exec(
      `SNX_NETWORK=bsc SUBGRAPH=${subgraph} ./node_modules/.bin/graph codegen ./subgraphs/${subgraph}.js -o ./generated/subgraphs/${subgraph}`,
    );
  }
    ...
    if (settings.network == 'All') {
      ...
      if (!settings.buildOnly) {
        await exec(
          `${prefixArgs} ./node_modules/.bin/graph deploy --node https://api.thegraph.com/deploy/ --ipfs https://api.thegraph.com/ipfs/ ${
            settings.team
          }/new-chapel-issuance1 ./subgraphs/${settings.subgraph}.js`,
        );
        // await exec(
        //   `${prefixArgs} ./node_modules/.bin/graph deploy --node https://api.thegraph.com/deploy/ --ipfs https://api.thegraph.com/ipfs/ ${
        //     settings.team
